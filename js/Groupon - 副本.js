            ...
            news.onreadystatechange = function (argument) 
            {
            	if (news.readyState == 4) 
				{
					...
					for (var i = 0; i < d3.length; i++) 
					{
						...
						{
							type:"get",
							url:"http://juhuituan.boguyuan.com/juhuituan/reqData?action=listCate&acode=1&uid=25177&type=3&id=" +d3[i].id,
							dataType:"json",
							async:false,
							success:function (data) 
							{
								var data = data.data;
								// console.log(data)
								that.sub_all.push(data);
							}
						});

                        
					}
					console.log(that.sub_all);
